...
class Post:
    ...
    def report(self):
        ...
        """
        import datetime
        with open("./blog_report_" + "{0:%Y-%m-%d_%H-%M-%S}.txt".format(datetime.datetime.now()), "a") as file:
...
if __name__ == "__main__":
    post = Post("Egor", "Prog-5", "Davaite pogovorim")
    comment1 = Comment("Kunica", "Hello", "Ya tuta!")
    comment1_1 = Comment("Egor", "Hacker", "Nu ya zhe hacker")
    comment1_2 = Comment("Kunica", "Best", "U R T Best")
    comment1.add_comment(comment1_1)
    comment1.add_comment(comment1_1)  # не сработает потому что add_comment проверяет что делает
    comment1.add_comment(comment1_2)
    comment1_1.add_comment(comment1_1)  # не сработает потому что add comment проверяет что делает
